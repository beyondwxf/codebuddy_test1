  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" class="search-bar">
      ...
      <el-form-item label="公司名称" prop="companyName">
        <el-input v-model="queryParams.companyName" placeholder="请输入公司名称" clearable style="width: 180px" @keyup.enter="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="联系状态" prop="contactStatus">
        <el-select v-model="queryParams.contactStatus" placeholder="联系状态" clearable style="width: 130px">
          <el-option
    ...
    <!-- 数据表格 -->
    <el-table v-loading="loading" :data="supplierList" @selection-change="handleSelectionChange" stripe border>
      ...
      <el-table-column label="电话1" prop="phone1" width="130" show-overflow-tooltip />
      <el-table-column label="电话2" prop="phone2" width="130" show-overflow-tooltip />
      <el-table-column label="电话3" prop="phone3" width="130" show-overflow-tooltip />
      <el-table-column label="电话4" prop="phone4" width="130" show-overflow-tooltip />
      <el-table-column label="联系状态" prop="contactStatus" width="100" align="center">
        <template #default="{ row }">
          <el-tag :type="statusTagType(row.contactStatus)" size="small">
            {{ statusLabel(row.contactStatus) }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="品类" prop="category" min-width="140" show-overflow-tooltip />
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="650px" append-to-body destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="联系状态" prop="contactStatus">
              <el-select v-model="form.contactStatus" placeholder="请选择联系状态" style="width: 100%">
                <el-option
                  v-for="item in contactStatusOptions"
                  :key="item.dictValue"
                  :label="item.dictLabel"
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="电话4" prop="phone4">
              <el-input v-model="form.phone4" placeholder="请输入电话4" />
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <!-- 导入弹窗 -->
    <el-dialog title="供应商数据导入" v-model="importVisible" width="400px" append-to-body destroy-on-close>
      <el-upload
        ref="uploadRef"
        :action="importUrl"
...
import { listSupplier, getSupplier, addSupplier, updateSupplier, delSupplier, exportSupplier, importSupplier, downloadTemplate } from '@/api/system/supplier'
...
const contactStatusOptions = ref([])
...
const importUrl = '/dev-api/api/system/supplier/import'
const uploadHeaders = computed(() => ({ Authorization: 'Bearer ' + getToken() }))

// 表单校验规则
const rules = {
  companyName: [{ required: true, message: '请输入公司名称', trigger: 'blur' }],
  contactStatus: [{ required: true, message: '请选择联系状态', trigger: 'change' }]
}
...
const statusLabel = (val) => {
  const item = contactStatusOptions.value.find(d => d.dictValue === val)
...
const loadDictData = async () => {
  try {
    const [provinceRes, categoryRes, statusRes] = await Promise.all([
      getDicts('supplier_province'),
      getDicts('supplier_category'),
      getDicts('supplier_contact_status')
    ])
    provinceOptions.value = provinceRes.data || []
    categoryOptions.value = categoryRes.data || []
    contactStatusOptions.value = statusRes.data || []
  } catch {
    // 字典加载失败不影响主流程
  }
}
...
const getList = async () => {
  ...
  try {
    const params = {
      ...
      category: queryParams.categoryList.length > 0 ? queryParams.categoryList.join(',') : undefined,
      contactStatus: queryParams.contactStatus || undefined
    }
...
const reset = () => {
  form.value = {
    supplierId: null, companyName: '', province: '', contactPerson: '',
    phone1: '', phone2: '', phone3: '', phone4: '',
    contactStatus: '0', categoryList: [], category: '',
    address: '', remark: ''
  }
...
const handleUpdate = async (row) => {
  reset()
  const res = await getSupplier(row.supplierId)
  const data = res.data
  // category字符串转数组
  data.categoryList = data.category ? data.category.split(',').filter(Boolean) : []
...
const submitForm = () => {
  formRef.value.validate(async (valid) => {
    if (!valid) return
    // categoryList数组转逗号分隔字符串
    const submitData = { ...form.value }
    submitData.category = submitData.categoryList ? submitData.categoryList.join(',') : ''
...
const handleDelete = async (row) => {
  const ids = row.supplierId ? row.supplierId : selectedIds.value.join(',')
  await ElMessageBox.confirm('是否确认删除选中的供应商数据？', '提示', { type: 'warning' })
...
const handleExport = async () => {
  try {
    await ElMessageBox.confirm('是否确认导出供应商数据？', '提示', { type: 'warning' })
    const params = {
      companyName: queryParams.companyName || undefined,
      province: queryParams.provinceList.length > 0 ? queryParams.provinceList.join(',') : undefined,
      contactPerson: queryParams.contactPerson || undefined,
...
const handleImportSuccess = (response) => {
  // el-upload on-success 回调参数直接是响应体（Result 对象）
